...
export function useCardLimits() {
  ...
  const canDrawCard = useCallback((player: Player, toReserve: boolean = false): boolean => {
  ...
  const handleDiscard = useCallback((player: Player, card: Card): Player => {
    ...
    return {
      ...player,
      hand: isInHand ? player.hand.filter(c => c.id !== card.id) : player.hand,
      reserve: isInReserve ? player.reserve.filter(c => c.id !== card.id) : player.reserve,
      discardPile: [...player.discardPile, card]
    };
  }, []);

  const handleDraw = useCallback((player: Player, deck: Card[], toReserve: boolean = false): { player: Player; deck: Card[] } => {
