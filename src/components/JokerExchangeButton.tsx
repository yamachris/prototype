...
export function JokerExchangeButton() {
  ...
  const { currentPlayer, phase, hasPlayedAction, columns, displayJokerExchangePopup, turn } = useGameStore();
  ...
  useEffect(() => {
    ...
    var availableCardsHand: Card[] = [];
    var availableCardsReserve: Card[] = [];
      ...
      whereJokerArePresent.map((position) => {
        availableCardsHand.push(
          ...currentPlayer.hand.filter(
            (card) => card.suit === position.suit && card.value == (position.index + 1).toString()
          )
        );
    ...
    if (availableCardsHand.length + availableCardsReserve.length > 0) {
      let temp = availableCardsHand;
      temp.push(...availableCardsReserve);
      setAvailablecards(temp.map((card: Card) => ({ ...card, isSelected: false })));
