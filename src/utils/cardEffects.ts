...
export type CardEffect = {
  type: 'heal' | 'damage' | 'special';
  value: number;
  description: string;
};

export function getCardEffect(card: Card, column?: ColumnState): CardEffect | null {
  if (card.value === 'JOKER') {
    return {
      type: 'special',
      value: 3,
...
export function handleCardEffect(card: Card, column: ColumnState, player: Player): Player {
...
export function shouldAutoDiscard(card: Card, column: ColumnState): boolean {
  ...
  if (card.value === '7' && column.cards.some(c => c?.value === 'Q')) return true;
