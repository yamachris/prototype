...
export function Card({
  ...
  return (
    <div className="relative">
      <button
        ...
        <div className="relative w-full h-full p-1">
          ...
          <div className="absolute top-1 left-1 flex flex-col items-center">
            <span
              className={cn(textSizes[size], "font-bold", isRed ? "text-red-500" : "text-gray-800 dark:text-gray-200")}>
              {card.value}
            </span>
            {!isJoker && getSuitIcon(cn("w-4 h-4", isRed ? "text-red-500" : "text-gray-800 dark:text-gray-200"))}
          </div>
      ...
      {/* JOKER Actions */}
      {isJoker && isSelected && isPlayerTurn && (
        <JokerActions card={card} onAction={onJokerAction} isPlayerTurn={isPlayerTurn} />
      )}
      ...
      {card.value === "Q" && isSelected && isPlayerTurn && selectedCards && (
        <QueenActions
          queen={card}
          activator={selectedCards.find((c) => c.type === "JOKER" || c.value === "7")}
